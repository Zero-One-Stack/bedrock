---
description: Review the current frontend changes against this kit's engineering philosophy (Feature-Sliced Design, engine-agnostic styling with tokens recommended, React Query/RHF+Zod, testing rules, no Effector/Redux, strict TS).
---
...
component contract, the **styling engine recorded in `project-specifics.md`** (and, when tokens
are in use, flag literal CSS values, primitive-token use in components, and missing state
siblings on interactive surfaces — see `styling-engine.md` + `styling-and-tokens.md`), React
Query / RHF+Zod usage, the 13 testing rules + coverage + behavior-not-implementation,
strict-TypeScript (no `any`), accessibility, i18n, and the hard bans (**no Effector / Redux for
server state**).
