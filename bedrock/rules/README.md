...
| `working-cadence.md`          | The four-phase working rhythm: brainstorm → plan (`/architect`) → execute (scaffolders + `component-builder`) → review (`/verify-build` + `frontend-reviewer`). Light methodology that strings the kit's existing agents together; complements (doesn't replace) superpowers / compound-engineering. | Non-trivial change spanning >1 component. |
...
| `design-system-structure.md`  | **The `shared/ui` design-system layout**: atomic grouping (`atoms/molecules/organisms`) as the **default**, every component a folder owning its colocated test + story, the test-runner-glob requirement, and the flat→folder migration ratchet. | Adding or placing a design-system component. |
...
| `cross-slice-communication.md` | **The cross-feature event escape hatch.** When compose-from-above can't reach (fan-out, distance, cross-cutting observers), a typed event bus on the `shared` floor (`shared/lib/events/`) lets slices coordinate without importing each other. Events are past-tense facts, never state/commands/request-response — does NOT reopen the Effector ban. The decision rule (when events vs. compose-from-above vs. push-down vs. React Query invalidate). | A feature must react to another feature's action and no single parent owns both. |
| `component-composition.md`    | Multi-part component namespace exports (Tabs/Dialog/Menu); polymorphism via `asChild` + Slot (banning raw `as`); `*.behavior.ts` split when interaction logic ≥30 LOC; variant API recipe pattern; one approved headless behavior library per repo (Base UI / Radix / React Aria / Ariakit) wrapping behavior in token-styled atoms. | Composing a multi-part component; choosing/using the headless behavior layer. |
| `form-primitives.md`          | The 15-primitive baseline every repo must ship in `shared/ui/` before any feature builds a form: Field, Label, HelpText, ErrorText, Input, Textarea, NumberInput, Select, Combobox, Checkbox, Radio, Switch, Slider, DateField, FileInput. RHF + Zod integration; ARIA wiring owned by `<Field>`. | Building a form; missing a form primitive. |
| `storybook.md`                | CSF3 + autodocs on every component; `play` for interactive atoms; FSD-mirrored title convention; theme × viewport × forced-colors × LTR/RTL matrix configured globally in `preview.ts`; a11y addon blocking CI; visual regression (Chromatic OR Playwright snapshots — pick one). | Writing or reviewing Storybook coverage. |
| `icon-system.md`              | One typed `<Icon name="…" size="…" />` atom per repo, reading from a build-time SVG sprite; `currentColor` fill; size tokens; `aria-hidden` by default, `aria-label` for meaningful icons; multiple icon libraries forbidden. | Adding an icon; choosing an icon library. |
...
| `theming.md`                  | The theming **contract** every engine must satisfy: `data-theme` + optional `data-brand` source on `<html>`, SSR flash prevention via cookie + inline head script, `prefers-color-scheme` reconciliation, `forced-colors` (Windows High Contrast), RTL via logical properties, multi-brand keying. Per-engine notes (CSS Modules, Tailwind, Chakra v3, vanilla-extract / Panda, runtime CSS-in-JS). | Wiring dark mode, brand switching, forced-colors, or RTL. |
...
| `react-19-and-server-actions.md` | **The Server Action security contract** (validate → authenticate → authorize → mutate → revalidate; a Server Action is a public endpoint; the closure is not a boundary) + the **Data Access Layer**; React 19 client model (`useActionState`, `useOptimistic`, `useFormStatus`, `use()`, ref-as-prop, React Compiler); Next 15/16 pins (async `params`/`cookies`, `proxy.ts`, `'use cache'`). | Writing any mutation or Server Action; wiring a form's pending/error state; reading a promise in a client component. |
